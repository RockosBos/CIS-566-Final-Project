...
export default function GameManager(props:any){
	...
	const [playerCardState, setPlayerCardState] = useState(["1.0", "1.0", "1.0"]);
	const [opponentCardState, setOpponentCardState] = useState(["1.0", "1.0", "1.0"]);
	...
	useEffect(() => {
		if(selectedPlayerCard != undefined && selectedOpponentCard != undefined){
			...
			if(attackResult > 60){
				let tempOpponentCardState = opponentCardState;
				opponentCards.forEach((e:any, index:any) => {
					if(e.CardID == opponent.CardID){
						tempOpponentCardState[index] = "0.0";
						setOpponentCardState(tempOpponentCardState);
					}
				});
			}
			else{
			...
			let randCardSelect = Math.floor(Math.random() * 3);
			let randWinProbability = Math.floor(Math.random() * 10);
			let tempPlayerCardState = playerCardState;

			if(randWinProbability > 5){
				tempPlayerCardState[randCardSelect] = "0.0";
				setPlayerCardState(tempPlayerCardState);
			}

		console.log(playerCardState);
		console.log(opponentCardState);
		if(playerCardState[0] == "0.0" && playerCardState[1] == "0.0" && playerCardState[2] == "0.0"){
			props.setResult("L");
			props.setShowResults(true);
			props.setShowGame(false);
			console.log("Loss");
		}
		else if(opponentCardState[0] == "0.0" && opponentCardState[1] == "0.0" && opponentCardState[2] == "0.0"){
			props.setResult("W");
			props.setShowResults(true);
			props.setShowGame(false);
			console.log("Winner");
	...
	}, [playerCardState, opponentCardState])
	...
	return(
		...
		<div className="GameScreen">
			...
			<div className="opponent">
				<div className="opponentCard" onClick={() => opponentCardClick(0)} style={{border: `5px solid ${opponentCardBorders[0]}`, opacity: `${opponentCardState[0]}`}}>
				...
				<div className="opponentCard" onClick={() => opponentCardClick(1)} style={{border: `5px solid ${opponentCardBorders[1]}`, opacity: `${opponentCardState[1]}`}}>
				...
				<div className="opponentCard" onClick={() => opponentCardClick(2)} style={{border: `5px solid ${opponentCardBorders[2]}`, opacity: `${opponentCardState[2]}`}}>
			...
			<div className="player">
				<div className="playerCard" onClick={() => playerCardClick(0)} style={{border: `5px solid ${playerCardBorders[0]}`, opacity: `${playerCardState[0]}`}}>
				...
				<div className="playerCard" onClick={() => playerCardClick(1)} style={{border: `5px solid ${playerCardBorders[1]}`, opacity: `${playerCardState[1]}`}}>
				...
				<div className="playerCard" onClick={() => playerCardClick(2)} style={{border: `5px solid ${playerCardBorders[2]}`, opacity: `${playerCardState[2]}`}}>
